...
**Model settings (recommendation):** low temperature (for example `0.2`), JSON mode if available for your SDK.
...
## User message template (application-provided)

The Express route should build:
...
```json
{
  ...
  "context": {
    "averageSessionMinutesTarget": 9
  }
}
```

Map request fields from [api-spec.md](api-spec.md) into this object.

---

## Validation

After generation, validate with [hardware-protocol.schema.json](hardware-protocol.schema.json). On failure, retry up to **two** additional times with the same system prompt; then apply server-side fallback protocol.
